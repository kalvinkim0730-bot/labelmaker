...
# =========================================================================
# [기존 기능] 1. 엑셀 파일 업로드 라벨 생성기 (식별 라벨 엔진 탑재)
# =========================================================================
...
if uploaded_file:
    ...
    if st.button("🚀 Generate 100x80mm PDF (Excel)", use_container_width=True):
        ...
        for index, row in df.iterrows():
            ...
            # [신규 핵심] 다음 품목으로 넘어가기 직전에 "식별 라벨 간지" 1장 삽입 (맨 마지막 줄 뒤에는 안 생기도록 예외 처리)
            if index < total_rows - 1:
                # 식별할 수 있도록 테두리를 점선이나 더 두꺼운 실선으로 차별화
                margin = 0.1 * inch
                c.setLineWidth(4) # 식별용 굵은 테두리
                c.rect(margin, margin, width - (2*margin), height - (2*margin))
                
                # 중앙에 식별 안내 문구와 품목코드 마킹
                current_code = str(row.iloc[0]).strip() if pd.notna(row.iloc[0]) else "UNKNOWN"
                next_row = df.iloc[index + 1]
                next_code = str(next_row.iloc[0]).strip() if pd.notna(next_row.iloc[0]) else "UNKNOWN"
                
                c.setFont(k_font, 18)
                c.drawCentredString(width/2, height - 0.8*inch, "▼ --------- NEXT ITEM --------- ▼")
                
                # 다음 나올 품목코드를 대대적으로 박아서 완충 구역 생성
                c.setFont(k_font, 32)
                c.drawCentredString(width/2, height/2 - 0.1*inch, next_code)
                
                c.setFont(k_font, 14)
                c.drawCentredString(width/2, 0.6*inch, "여기서부터 다음 품목 라벨입니다.")
...
# =========================================================================
# [기존 기능] 2. 라벨 수동 생성 (항목 직접 입력 모듈) - 순정 유지
# =========================================================================
